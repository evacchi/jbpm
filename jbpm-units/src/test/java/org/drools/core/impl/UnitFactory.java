...
public class UnitFactory {
    ...
    public UnitFactory bindVariable(String name, Object dataSource ) {
        variables.put( name, dataSource );
        return this;
    }

    public RuleUnit getOrCreateRuleUnit( AbstractUnitExecutor executor, String name, ClassLoader classLoader ) {
        try {
    ...
    public <T extends RuleUnit> T getOrCreateRuleUnit( AbstractUnitExecutor executor, Class<T> ruleUnitClass ) {
        return (T)units.computeIfAbsent( new RuleUnit.Identity(ruleUnitClass), id -> createRuleUnit( executor, ruleUnitClass ) );
    }

    public RuleUnit registerUnit( AbstractUnitExecutor executor, RuleUnit ruleUnit ) {
        return units.computeIfAbsent( ruleUnit.getUnitIdentity(), id -> injectUnitVariables( executor, ruleUnit ) );
    }

    private <T extends RuleUnit> T createRuleUnit( AbstractUnitExecutor executor, Class<T> ruleUnitClass ) {
        try {
    ...
    public <T extends RuleUnit> T injectUnitVariables( AbstractUnitExecutor executor, T ruleUnit ) {
        return injectUnitVariables( executor, (Class<T>) ruleUnit.getClass(), ruleUnit );
    }

    private <T extends RuleUnit> T injectUnitVariables( AbstractUnitExecutor executor, Class<T> ruleUnitClass, T ruleUnit ) {
